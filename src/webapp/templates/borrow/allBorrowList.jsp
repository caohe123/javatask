...
<head>
    ...
    <style>
        * {
        ...
        .status-normal { color: #1971c2; font-weight: 500; }
        .status-returned { color: #2f9e44; font-weight: 500; }
        .status-overdue { color: #e03131; font-weight: bold; }
        .btn {
        ...
        .btn-return { background-color: #e03131; }
    ...
    <c:if test="${not empty borrowPage.list and borrowPage.totalCount > 0}">
                    ...
                    <td>
                        ...
                        <c:if test="${borrow.status == 0 or borrow.status == 2}">
                            <a href="${pageContext.request.contextPath}/borrow?action=returnBook&borrowId=${borrow.id}"
                               ...
                               onclick="return confirm('确认帮该用户归还图书吗？')">
                                办理归还
                            </a>
                        </c:if>
        ...
        <div class="pagination">
            <c:if test="${borrowPage.currentPage > 1}">
                <a href="${pageContext.request.contextPath}/borrow?action=allList&pageNum=1">首页</a>
                <a href="${pageContext.request.contextPath}/borrow?action=allList&pageNum=${borrowPage.currentPage - 1}">上一页</a>
            </c:if>

            <c:forEach begin="1" end="${borrowPage.totalPage}" var="i">
                <a href="${pageContext.request.contextPath}/borrow?action=allList&pageNum=${i}"
                   class="${borrowPage.currentPage == i ? 'active' : ''}">${i}</a>
            </c:forEach>

            <c:if test="${borrowPage.currentPage < borrowPage.totalPage}">
                <a href="${pageContext.request.contextPath}/borrow?action=allList&pageNum=${borrowPage.currentPage + 1}">下一页</a>
                <a href="${pageContext.request.contextPath}/borrow?action=allList&pageNum=${borrowPage.totalPage}">尾页</a>
            </c:if>
        </div>
    </c:if>
